...
class PrinterManager {
  ...
  async print(receiptText, printerName = null) {
    try {
      ...
      try {
        // SIMPLIFIED: Use simple lp command like your successful terminal test
        // No raw mode, no complex options - just direct printing
        const printCommand = `LC_ALL=C lp -d ${targetPrinter} "${tempFile}"`;
  ...
  async testPrinter(printerName) {
    try {
      // Test text with ESC/POS commands like the receiptFormatter uses
      const testText = `\x1B@\x1B!\x38
================================
        PRINTER TEST
================================
